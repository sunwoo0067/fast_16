...
# 데이터베이스 설정
DATABASE_URL = "postgresql+asyncpg://postgres:12345678@localhost:5433/fast_15"
...
class Product(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    supplier_id = Column(Integer, index=True, nullable=False)
    item_key = Column(String, index=True, nullable=False)  # W000000
    name = Column(String, nullable=False)
    model = Column(String)
    brand = Column(String)
    category = Column(String)
    price = Column(Integer)  # 기본 가격
    options = Column(Text)  # JSON 형태로 옵션 정보 저장
    description = Column(Text)
    ...
    # 쿠팡 상품 관련 필드들
    coupang_product_id = Column(String, index=True)  # 쿠팡 상품 ID
    sale_price = Column(Integer)  # 판매 가격
    status = Column(String)  # 쿠팡 상품 상태
    category_id = Column(String)  # 쿠팡 카테고리 ID
    manufacturer = Column(String)  # 제조사
    coupang_product_data = Column(Text)  # JSON 형태로 쿠팡 상품 전체 데이터
    coupang_summary_data = Column(Text)  # JSON 형태로 쿠팡 상품 요약 데이터
